...
|---|---|---|
| `v0.10.0` | `time()` at READ time | intermittent — same file, two reads, ~1 in 5 |
| `v0.10.1` | CRC-32 of the whole package | the package embeds a save timestamp, so it followed the WRITER's clock: every save, every time |
| `v0.10.2` | CRC-32 of the package minus `docProps/core.xml` | removed the clock, kept a bytes-derived id |
| **v0.10.3** | **CRC-32 of the deck `read()` returns** | — |

v0.10.2 removed one source of byte variance and left the others. Two
...
Measured on the case that survived v0.10.2: a deck whose only element is a shape,
...
they do not. `read(foreign) vs read(toBytes(read(foreign)))` on a
LibreOffice-produced file — 19644 bytes in, 14168 out — likewise now agrees.

All three engines ship it together: `@particle-academy/dark-slide` 0.8.3 and
...
- **`tests/fixtures/foreign-libreoffice.pptx` — the first `.pptx` fixture this
  ...
  left the reader's `random_int()` path unexercised for several releases. It is
...
- **Renaming a deck changes its id again.** v0.10.2 had it not change, as a side
  ...
  against v0.10.2 and relied on a rename being id-neutral, it no longer is.

- **If you PERSISTED diffs, this is a data-format change** — the same clause as
  v0.10.2, now with one more scheme in the window. Read this only if you store
...
**A fix to a fix: 0.10.1 moved this defect rather than removing it.** That release
...
All three engines ship it together: `@particle-academy/dark-slide` 0.8.2 and
...
- **`read()` is a pure function of its bytes, including across a save.**
  ...
  `gmdate()` at write time. ``tests/Unit/ReaderIsPureTest.php`` covers it
